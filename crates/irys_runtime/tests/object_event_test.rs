...
fn test_object_event_handling() {
    ...
    // In `irys_runtime`, module fields are initialized when?
...
fn eval_str(interp: &mut Interpreter, expr_str: &str) -> Result<Value, irys_runtime::RuntimeError> {
    let expr = irys_parser::parse_expression_str(expr_str)
        .map_err(|e| irys_runtime::RuntimeError::Custom(format!("Parse error: {}", e)))?;
