...
fn test_array_functionality() {
    let source = std::fs::read_to_string("../../tests/test_arrays.vb").expect("Failed to read test file");
    match parse_program(&source) {
        Ok(program) => {
            ...
            assert!(has_success, "Array test failed - did not see SUCCESS");
            assert!(!has_failure, "Array test failed - saw FAILURE");
        },
        Err(e) => {
            panic!("Parse error: {:?}", e);
        }
