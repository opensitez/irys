...
#[test]
fn test_constants_functionality() {
    let source = std::fs::read_to_string("../../tests/test_constants.vb").expect("Failed to read test file");
    match parse_program(&source) {
        Ok(program) => {
            ...
            assert!(has_success, "Constants test failed - did not see SUCCESS");
            assert!(!has_failure, "Constants test failed - saw FAILURE");
        },
        Err(e) => {
            panic!("Parse error: {:?}", e);
        }
